import { IHasLte } from '../collection.interface';
import { Iterateable } from '../types';

export function testLte(create: <T>(iterable: Iterateable<T>) => IHasLte<T>): void {
  describe('lte(...)', () => {
...
function testBench(numbers: IHasLte<number>, dates: IHasLte<Date>) {
  expect(Array.from(numbers.lte(3))).toEqual([1, 2, 3,]);

  expect(Array.from(dates.lte(new Date('2010-01-01'))))
    .toEqual([
      new Date('1990-01-01'),
      new Date('2000-01-01'),
      new Date('2010-01-01'),
